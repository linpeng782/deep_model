...
def compare_csv_files(file1_path, file2_path):
    ...
    try:
        ...
        if '交易日期' in df1.columns:
            ...
            # 截取共同日期的数据
            df1 = df1[df1['交易日期'].astype(str).isin(common_dates)].reset_index(drop=True)
            df2 = df2[df2['交易日期'].astype(str).isin(common_dates)].reset_index(drop=True)
...
def find_all_differences(folder1=None, folder2=None):
    """
    找出所有有差异的文件
    
    Args:
        folder1 (str): 第一个文件夹路径，默认使用ENHANCED_DATA_DIR
        folder2 (str): 第二个文件夹路径，默认使用ENHANCED_DATA_DIR + "_20250819"
    """
    if folder1 is None:
    ...
    for stock_code in original_by_code.keys():
        if stock_code in new_by_code:
            common_files.append((original_by_code[stock_code].name, new_by_code[stock_code].name))
    ...
    # 按差异程度排序（从大到小）
    if different_files:
        different_files.sort(key=lambda x: x['diff_info'].get('overall_ratio', 0), reverse=True)
    ...
    report_lines.append(f"完全一致的文件: {identical_files} ({identical_files/len(common_files)*100:.2f}%)")
    report_lines.append(f"有差异的文件: {len(different_files)} ({len(different_files)/len(common_files)*100:.2f}%)")
    report_lines.append("")
    ...
    if different_files:
        ...
        # 显示前5名（排除退市股票）
        non_delisted_files = [f for f in different_files if f['diff_info']['type'] != 'no_common_dates']
        top5_files = non_delisted_files[:5] if len(non_delisted_files) >= 5 else non_delisted_files
        for i, diff_file in enumerate(top5_files, 1):
            ...
            if diff_info['type'] == 'numeric_diff':
                ...
                # 只显示前3个差异最大的列
                sorted_cols = sorted(diff_info['column_details'], key=lambda x: x['diff_ratio'], reverse=True)[:3]
        ...
        for i, diff_file in enumerate(different_files, 1):
            ...
            if diff_info['type'] == 'numeric_diff':
                ...
                console_output = f"    形状: {diff_info['shape1']} vs {diff_info['shape2']}"
                print(console_output)
                report_lines.append(console_output)
    ...
    report_dir = os.path.join(os.path.dirname(__file__), "..", "data", "comparison_reports")
...
def main():
    ...
    主函数，处理命令行参数
    """
    parser = argparse.ArgumentParser(description='比较两个文件夹中的CSV文件差异')
